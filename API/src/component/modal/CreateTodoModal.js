...
const CreateTodoModal = ({handleData}) => {
    ...
    const handleSubmit = async (values, setErrors) => {
        try {
            ...
            if(jwtToken){
                const result = await todoService.createTodo(values,jwtToken);
                let submitModal = document.getElementById("closeModalCreate");
                submitModal.click();
                handleData(result.id);
                Swal.fire({
                    title: 'Create todo '+result.name+' success',
                    icon: 'success',
                })
            }
        } catch (error) {
